...
namespace
{
	...
	struct TPlayerBotSidekick
	{
		...
		TPlayerBotSidekick()
			: dwOwnerPID(0), dwSidekickPID(0), bMode(PLAYERBOT_SIDEKICK_FOLLOW),
			  bStance(PLAYERBOT_SIDEKICK_STANCE_ATTACK), bGroup(0), bLevel(1), bSetupDone(true), dwOwnerSeenAt(0),
			  dwNextSpawnTry(0)
		{
		}
	};
	...
	bool s_bPlayerBotSidekickStanceColumn = false;
	...
	struct TPlayerBotSidekickRuntime
	{
		...
		TPlayerBotSidekickRuntime()
			: dwNextPartyCheck(0), dwNextService(0), dwNextLoot(0), dwNextCatchUp(0), dwLootVID(0),
			  dwLootSince(0), dwNextProtect(0), bTrading(false), dwLastFoeVID(0)
		{
	...
	bool EnsurePlayerBotSidekickTable()
	{
		...
		// The stance came a day after the table, which the test world already
		// held; without the column the companions still load, and fight in the
		// default stance.
		std::unique_ptr<SQLMsg> stance(AccountDB::instance().DirectQuery(
				"ALTER TABLE player.playerbot_sidekick ADD COLUMN IF NOT EXISTS "
				"stance TINYINT UNSIGNED NOT NULL DEFAULT 0 AFTER mode"));
		s_bPlayerBotSidekickStanceColumn = stance.get() && stance->uiSQLErrno == 0;
		if (!s_bPlayerBotSidekickStanceColumn)
			sys_err("PLAYERBOT_SIDEKICK: no stance column errno=%u", stance.get() ? stance->uiSQLErrno : 0U);
	...
	void LoadPlayerBotSidekicks()
	{
		if (!EnsurePlayerBotSidekickTable())
			return;
		std::unique_ptr<SQLMsg> msg(AccountDB::instance().DirectQuery(s_bPlayerBotSidekickStanceColumn
				? "SELECT owner_pid, sidekick_pid, mode, skill_group, start_level, setup_done, stance "
				  "FROM player.playerbot_sidekick"
				: "SELECT owner_pid, sidekick_pid, mode, skill_group, start_level, setup_done, 0 "
				  "FROM player.playerbot_sidekick"));
		...
		while (NULL != (row = mysql_fetch_row(msg->Get()->pSQLResult)))
		{
			...
			unsigned int mode = 0, group = 0, level = 1, done = 0, stance = 0;
			...
			if (old != s_mapPlayerBotSidekicks.end() && old->second.dwSidekickPID == rec.dwSidekickPID)
			{
				...
				// With no column to keep it in, the stance lives as long as the core.
				if (!s_bPlayerBotSidekickStanceColumn)
	...
	// (IsPlayerBotHeldForCompany), so none of the errands take it away.
	...
	// call back, a report and, after half an hour, the dismissal - every path
	// a person's letter takes, on a world with no person in it.
	void StepPlayerBotSidekickSelfTest(DWORD dwNow)
	{
		...
		sys_log(0, "PLAYERBOT_SIDEKICK: self-test step=%d owner=%u sidekick=%u here=%d map=%ld/%ld dist=%d party=%d",
	...
	void RunPlayerBotSidekickSelfTest(DWORD dwNow)
	{
		...
		// after a restart rather than a second one made.
	...
	void ManagePlayerBotSidekicks(DWORD dwNow)
	{
		...
		for (TPlayerBotSidekickMap::iterator it = s_mapPlayerBotSidekicks.begin(); it != s_mapPlayerBotSidekicks.end(); ++it)
		{
			...
			else if (here && (rec.dwOwnerSeenAt == 0 || dwNow - rec.dwOwnerSeenAt >= PLAYERBOT_SIDEKICK_OWNER_GONE_MS))
			{
	...
	// letter - "chodz", "graj sam", "stan", and how it fights ("atakuj", "nie
	// atakuj pierwszy", "nie walcz"; one whisper may carry a stance and a
	// call). Anything else is a conversation, and the conversation layer
	// answers it as it answers anybody (playerbot_chat_conversation.h).
	bool HandlePlayerBotSidekickWhisper(LPCHARACTER from, LPCHARACTER bot, const char* text)
	{
	...
	// /towarzysz stworz <rasa 0-7> <sciezka 1-2> <nick> | przywolaj | wolny | stan | odprawa tak
	//            | walka <0 atakuj, 1 nie atakuj pierwszy, 2 nie walcz>
	...
	struct FPlayerBotSidekickFoes
	{
		...
		FPlayerBotSidekickFoes(LPCHARACTER s, LPCHARACTER o)
			: self(s), owner(o), onOwner(NULL), onOwnerDist(INT_MAX), onSelf(NULL), onSelfDist(INT_MAX),
			  idle(NULL), idleDist(INT_MAX)
		{
		}
		...
		void operator()(LPENTITY ent)
		{
			...
			if (c == self || c == owner || c->IsDead() || !c->IsMonster() || c->GetMapIndex() != owner->GetMapIndex())
				return;
			const int fromOwner = DISTANCE_APPROX(c->GetX() - owner->GetX(), c->GetY() - owner->GetY());
			if (fromOwner > PLAYERBOT_SIDEKICK_GUARD_RANGE)
				return;
			const int fromSelf = DISTANCE_APPROX(c->GetX() - self->GetX(), c->GetY() - self->GetY());
			if (c->GetVictim() == owner)
			{
	...
	LPCHARACTER FindPlayerBotSidekickFoe(LPCHARACTER ch, LPCHARACTER owner, BYTE stance, int& why)
	{
		...
		if (!owner->GetSectree())
			return NULL;
		FPlayerBotSidekickFoes foes(ch, owner);
	...
	struct FPlayerBotSidekickLoot
	{
		...
		FPlayerBotSidekickLoot(LPCHARACTER s, LPCHARACTER o, const TPlayerBotSidekickRuntime& r, DWORD n)
			: self(s), owner(o), rt(r), now(n), best(NULL), bestDist(INT_MAX)
		{
		}
		...
		void operator()(LPENTITY ent)
		{
			...
			if (failed != rt.mapLootFailed.end() && (int)(now - failed->second) < 0)
				return;
			const bool ownersOnly = item->IsOwnership(owner) && !item->IsOwnership(self);
			if (ownersOnly)
			{
			...
			else if (!item->IsOwnership(self) || IsPlayerBotLootBeneathBot(self, item) ||
					!PlayerBotBagTakesDrop(self, item))
				return;
			best = item;
			bestDist = d;
		}
			...
			TPlayerBotSidekickRuntime& rt, DWORD dwNow)
	{
		...
		FPlayerBotSidekickLoot loot(ch, owner, rt, dwNow);
	...
	bool ManagePlayerBotSidekick(LPCHARACTER ch, TPlayerBotAIState& state, DWORD dwNow)
	{
		...
		if (foe)
		{
			...
			state.dwLastMeaningfulActivityTime = dwNow;
			// A map with no navigation grid: the fight's approach is a straight
			// walk, which the fight itself cannot make there.
		...
		if (dwNow >= rt.dwNextLoot)
		{
			...
			if (PickUpPlayerBotSidekickLoot(ch, state, owner, rt, dwNow))
				return true;
		}
		...
		if (ch->IsStateMove() && dist <= PLAYERBOT_SIDEKICK_FOLLOW_DISTANCE / 2)
			ch->Stop();
		state.dwLastMeaningfulActivityTime = dwNow;
		if (ManagePlayerBotBuffPerson(ch, state, owner, dwNow))
			return true;
